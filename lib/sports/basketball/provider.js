...
async function fetchGamesByDate(date, apiKey) {
  const url = `${API_BASE}/games?dates[]=${date}&per_page=100`;
  const data = await fetchPage(url, apiKey);
  return (data.data || [])
    .filter((g) => g.home_team.id <= 30 && g.visitor_team.id <= 30)
    .map((g) => ({
      homeTeamName: g.home_team.full_name, awayTeamName: g.visitor_team.full_name,
      date: g.date, status: g.status_state || g.status,
    }));
}

module.exports = { fetchSeasonGames, fetchGamesByDate };
