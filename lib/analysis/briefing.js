...
const SPORT_CONFIG = {
  football: { unit: 'goals', hasDraw: true, risk: 'Football is low-scoring with high single-match variance; any of the three outcomes remains plausible.' },
  basketball: { unit: 'points', hasDraw: false, risk: 'Basketball outcomes carry real game-to-game variance — a single game can swing on shooting variance or foul trouble regardless of season-long form.' },
  nfl: { unit: 'points', hasDraw: false, risk: 'A single NFL game carries real variance from turnovers, injuries, and game-flow swings regardless of season-long form.' },
  mlb: { unit: 'runs', hasDraw: false, risk: 'A single baseball game carries real variance from pitching matchups and bullpen performance that a season-long run rate does not capture.' },
  ufc: { unit: 'composite score', hasDraw: false, risk: 'A single fight can turn on one strike or one mistake — this model has no real backtested track record yet, so treat any lean here as a starting point, not a settled read.' },
};

function fallbackBriefing(evidence) {
  const { probabilities, fixture, lambdas, usedPriorSeason, sport } = evidence;
  const config = SPORT_CONFIG[sport] || SPORT_CONFIG.football;
  const hasDraw = config.hasDraw && probabilities.draw !== undefined;

  const favored = probabilities.home > probabilities.away
    ? `${fixture.home} (${probabilities.home}%)`
    : `${fixture.away} (${probabilities.away}%)`;
  const drawClause = hasDraw ? `, with a draw at ${probabilities.draw}%` : '';
  const conclusionDraw = hasDraw ? `, Draw ${probabilities.draw}%` : '';

  return {
    modelLean: `The model assigns the higher probability to ${favored}${drawClause}.`,
    ...
    primarySignal: `Expected ${config.unit}: ${fixture.home} ${lambdas.home} vs ${fixture.away} ${lambdas.away}.`,
    biggestRisk: config.risk,
    conclusion: `Probabilities: Home ${probabilities.home}%${conclusionDraw}, Away ${probabilities.away}%.`,
    driversFor: [],
