...
async function callNvidia(apiKey, instructions) {
  ...
  for (const model of NVIDIA_MODEL_CANDIDATES) {
    ...
    // A 404 means only this specific model isn't wired up right now —
    // try the next candidate rather than giving up on the whole provider.
    if (res.status === 404) continue;
    // Any other status (auth, rate limit, server error) applies account-wide,
    // not per-model — no point trying more candidates.
    break;
  }
