...
function fallbackBriefing(evidence) {
  ...
  const unit = isBasketball ? 'points' : 'goals';
  const riskText = isBasketball
    ? 'Basketball outcomes carry real game-to-game variance — a single game can swing on shooting variance or foul trouble regardless of season-long form.'
    : 'Football is low-scoring with high single-match variance; any of the three outcomes remains plausible.';
  ...
  return {
    ...
    primarySignal: `Expected ${unit}: ${fixture.home} ${lambdas.home} vs ${fixture.away} ${lambdas.away}.`,
    biggestRisk: riskText,
