...
async function fetchFinishedMatches(competitionCode, apiKey, season) {
  const cacheKey = `${competitionCode}:${season || 'current'}`;
  const cached = cache.get(cacheKey);
  if (cached && Date.now() - cached.fetchedAt < CACHE_TTL_MS) return cached.data;

  const seasonParam = season ? `&season=${season}` : '';
  const url = `${API_BASE}/competitions/${competitionCode}/matches?status=FINISHED${seasonParam}`;
  const res = await fetch(url, { headers: { 'X-Auth-Token': apiKey } });
  if (!res.ok) throw new Error(`http_${res.status}`);
  const data = await res.json();
  const result = { matches: data.matches || [], seasonYear: data.filters?.season || season || null };
