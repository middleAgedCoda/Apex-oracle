...
async function saveAnalysis({ competition, home, away, lambdas, probabilities, briefing, briefingSource }) {
  if (!pool) return null;
  const analysisId = genId('AO');
  const res = await pool.query(
    `INSERT INTO analyses (analysis_id, competition, home_team, away_team, lambdas, probabilities, briefing, briefing_source)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
    [analysisId, competition, home, away, JSON.stringify(lambdas), JSON.stringify(probabilities), JSON.stringify(briefing), briefingSource]
  );
  return res.rows[0];
}
